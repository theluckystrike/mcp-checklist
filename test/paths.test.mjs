...
test.skip("a pseudo-filesystem path fails fast instead of hanging the server", async (t) => {
  const { c } = open(t);
  await c.init();
...
test.skip("a normal nested path that does not exist yet is created, all levels of it", async (t) => {
  const { box, c } = open(t);
  await c.init();
  const { run } = await seed(c);
...
test.skip("a URL is refused by name and the refusal does not leak the cwd", async (t) => {
  const { c } = open(t);
  await c.init();
  const { run } = await seed(c);
...
test.skip("an existing file is refused, and overwrite replaces it", async (t) => {
  const { box, c } = open(t);
  await c.init();
  const { run } = await seed(c);
...
test.skip("the extension is added when it is missing and not doubled when it is there", async (t) => {
  const { box, c } = open(t);
  await c.init();
  const { run } = await seed(c);
...
test.skip("a tilde path expands to the home directory rather than being taken literally", async (t) => {
  const { c } = open(t);
  await c.init();
  const { run } = await seed(c);
